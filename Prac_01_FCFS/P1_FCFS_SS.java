...
public class P1_FCFS_SS

{

int burstTime[]; int arrivalTime[]; String[] processId;
int numberOfProcess;
...
if(at[j]>at[j+1]){ temp=at[j]; at[j]=at[j+1]; at[j+1]=temp; temp=bt[j]; bt[j]=bt[j+1]; bt[j+1]=temp; stemp=pid[j]; pid[j]=pid[j+1]; pid[j+1]=stemp; swapped=true;
}
...
int bt[]=burstTime.clone(); int at[]=arrivalTime.clone(); String pid[]=processId.clone();
...
turnAroundTime[0]=finishTime[0]-at[0]; waitingTime[0]=turnAroundTime[0]-bt[0]; for(int i=1;i<numberOfProcess;i++){ finishTime[i]=bt[i]+finishTime[i-1]; turnAroundTime[i]=finishTime[i]-at[i]; waitingTime[i]=turnAroundTime[i]-bt[i];
}
...
public static void main(String[] args){
 ...
 P1_FCFS_SS obj=new P1_FCFS_SS(); obj.getProcessData(input); 
